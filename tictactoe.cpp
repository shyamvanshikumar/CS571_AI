...
pair<int,vvi> Max_value(const vvi &, int, int);
pair<int,vvi> Min_value(const vvi &, int, int);
int terminal_check(const vvi &);

string Alpha_Beta_Search(const vvi &state, char player) {
    pair<int, vvi> move_pair = Max_value(state, -INF, INF);
...
pair<int, vvi> Max_value(const vvi &state, int alpha, int beta) {
    // static int maxcall=0;
    // cout<<maxcall++<<"\n";
    int t = terminal_check(state);
    if(t != -2)
        return {t, state};
    ...
    for(int i=0;i<n;i++) {
        for(int j=0;j<n;j++) {
            if(nstate[i][j] == '\0') {
                nstate[i][j] = 'x';
                pair<int, vvi> v2 = Min_value(nstate, alpha, beta);
                if(v2.first > v) {
                    v = v2.first;
                    ans = nstate;
                    alpha = max(alpha,v);
...
pair<int, vvi> Min_value(const vvi &state, int alpha, int beta) {
    ...
    for(int i=0;i<n;i++) {
        for(int j=0;j<n;j++) {
            if(nstate[i][j] == '\0') {
                nstate[i][j] = 'o';
                pair<int, vvi> v2 = Max_value(nstate, alpha, beta);
                if(v2.first < v) {
                    v = v2.first;
                    ans = nstate;
                    beta = min(beta,v);
...
int terminal_check(const vvi &state) {
    bool com = true;
    int n = state.size();
...
int main() {
    ...
    char turn = 'x';
    while(1) {
        int result = terminal_check(state);
        // cout<<"\n";
        // for(int i=0;i<n;i++) {
        //     for(int j=0;j<n;j++)
        //         cout<<state[i][j];
